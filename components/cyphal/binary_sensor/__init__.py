...
binary_sensor_ns = cyphal_ns.namespace("binary_sensor")

CyphalBinarySensorPublisher = binary_sensor_ns.class_("CyphalBinarySensorPublisher", cg.Component, binary_sensor.BinarySensor)
CyphalBinarySensorSubscriber = binary_sensor_ns.class_("CyphalBinarySensorSubscriber", cg.Component, binary_sensor.BinarySensor)
...
PLATFORM_SCHEMA = cv.Any(
    binary_sensor.binary_sensor_schema(CyphalBinarySensorSubscriber).extend({
    ...
    binary_sensor.binary_sensor_schema(CyphalBinarySensorPublisher).extend({
        cv.Required(CONF_CYPHAL_ID): cv.use_id(CyphalComponent),
        cv.Required(CONF_SUBJECT_ID): cv.int_range(min=0, max=8191),
        cv.Optional(CONF_DSDL_TYPE, default=DSDL_TYPE_BIT): cv.string,
        cv.Optional(CONF_SOURCE_SENSOR): cv.use_id(binary_sensor.BinarySensor),
