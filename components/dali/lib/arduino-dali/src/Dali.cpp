...
int DaliClass::sendRawWait(const byte * message, uint8_t bits, byte timeout) {
  ...
  while (!bus_.busIsIdle())
    if (::millis() - time > timeout) return DALI_READY_TIMEOUT;

  result = bus_.sendRaw(message, bits);

  while (!bus_.busIsIdle())
    if (::millis() - time > timeout) return DALI_READY_TIMEOUT;

  return (result != DALI_SENT) ? result : bus_.getLastResponse();
}
...
daliReturnValue DaliClass::sendArc(byte address, byte value, byte addr_type) {
  byte message[2];
  return bus_.sendRaw(prepareCmd(message, address, value, addr_type, 0), 16);
}
...
daliReturnValue DaliClass::sendCmd(byte address, DaliCmd command, byte addr_type) {
  byte message[2];
  return bus_.sendRaw(prepareCmd(message, address, command, addr_type, 1), 16);
}
...
daliReturnValue DaliClass::sendSpecialCmd(DaliSpecialCmd cmd, byte value) {
  ...
  byte message[2];
  return bus_.sendRaw(prepareSpecialCmd(message, command, value), 16);
}
...
void DaliClass::commission_tick() {
  ...
  if (bus_.busIsIdle()) { // wait until bus is idle
    switch (commissionState) {
      ...
      case COMMISSION_CHECKFOUND:
        ...
        int response = bus_.getLastResponse();
...
// DaliClass Dali; // Removed for multi-instance support
