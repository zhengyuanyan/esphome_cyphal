...
#define isDeltaWithin2TE(delta) (2*DALI_TE_MIN <= delta && delta <= 2*DALI_TE_MAX)
  ...
  #define getBusLevel (activeLow ? !(fastRead(rxPin)) : fastRead(rxPin))
  #define setBusLevel(level) fastWrite(txPin, (activeLow ? !level : level)); txBusLevel = level;
...
class DaliBusClass {
  public:
    ...
    {
      if(value)
        digitalWrite(pin, HIGH); // fallback for simplicity, or use GPIO
      else
    ...
    // Registry for ISRs
    static DaliBusClass* instances[2];
    static void IRAM_ATTR isr0();
    static void IRAM_ATTR isr1();
    static bool IRAM_ATTR timer_isr0(void* arg);
    static bool IRAM_ATTR timer_isr1(void* arg);
  ...
  protected:
    ...
    byte txMessage[4];
    uint8_t txLength;
    void* timer_obj_; // Pointer to Timer object
    ...
    volatile bool tempBusLevel = false;
    volatile uint16_t tempDelta = 0;
