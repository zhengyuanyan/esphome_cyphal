...
DaliBusClass* DaliBusClass::instances[2] = {nullptr, nullptr};

void IRAM_ATTR DaliBusClass::isr0() { if (instances[0]) instances[0]->pinchangeISR(); }
void IRAM_ATTR DaliBusClass::isr1() { if (instances[1]) instances[1]->pinchangeISR(); }
bool IRAM_ATTR DaliBusClass::timer_isr0(void* arg) { if (instances[0]) instances[0]->timerISR(); return true; }
bool IRAM_ATTR DaliBusClass::timer_isr1(void* arg) { if (instances[1]) instances[1]->timerISR(); return true; }

DaliBusClass::DaliBusClass(uint8_t id) : id_(id), timer_obj_(nullptr) {
  if (id < 2) instances[id] = this;
}
...
void DaliBusClass::begin(byte tx_pin, byte rx_pin, bool active_low) {
  ...
  if (id_ == 0) attachInterrupt(digitalPinToInterrupt(rxPin), isr0, CHANGE);
  else attachInterrupt(digitalPinToInterrupt(rxPin), isr1, CHANGE);

#if defined(ARDUINO_ARCH_RP2040)
  RPI_PICO_Timer* timer = new RPI_PICO_Timer(id_); // Use id_ as timer num
  timer_obj_ = timer;
  timer->attachInterrupt(2398, id_ == 0 ? (repeating_timer_callback_t)timer_isr0 : (repeating_timer_callback_t)timer_isr1);
#elif defined(ARDUINO_ARCH_ESP32)
  ESP32Timer* timer = new ESP32Timer(id_);
  timer_obj_ = timer;
  timer->attachInterrupt(2398, id_ == 0 ? timer_isr0 : timer_isr1);
#endif
}
...
#if defined(ARDUINO_ARCH_RP2040)
      if (timer_obj_) static_cast<RPI_PICO_Timer*>(timer_obj_)->restartTimer();
#elif defined(ARDUINO_ARCH_ESP32)
      if (timer_obj_) static_cast<ESP32Timer*>(timer_obj_)->restartTimer();
#endif
...
// DaliBusClass DaliBus; // Removed for multi-instance support